...
import { getScannerDetails } from "@/api/admin-api";
...
const ScannerInfo = () => {
    ...
    const { data, isLoading, isError, error } = useQuery({
        queryKey: ["scanner-details", id],
        queryFn: () => getScannerDetails(id),
    });
    console.log(data);
    ...
    if (isError) {
        return (
            <div className="text-center text-red-500 w-full">
                Error: {error?.message || "Failed to load scanner details"}
            </div>
        );
    }
                            ...
                            scannedDocuments.map((doc, idx) => (
                                <div key={doc.id}>
                                    <div className="flex items-center justify-between py-2">
                                        ...
                                        <a
                                            href={getStaticUrl(doc.filePath)}
                                            target="_blank"
                                    ...
                                    {idx < scannedDocuments.length - 1 && <Separator />}
                                </div>
                            ))
                        ) : (
