...
import { getUploaderDetails } from "@/api/admin-api";
...
const UploaderInfo = () => {
    ...
    const { data, isLoading, isError, error } = useQuery({
        queryKey: ["uploader-details", id],
        queryFn: () => getUploaderDetails(id),
    });
    ...
    if (isError) {
        return (
            <div className="text-center text-red-500 w-full">
                Error: {error?.message || "Failed to load uploader details"}
            </div>
        );
    }
                        ...
                        {uploadedDocuments.length > 0 ? (
                            uploadedDocuments.map((doc, idx) => (
                                <div key={doc.id}>
                                    ...
                                    {idx < uploadedDocuments.length - 1 && <Separator />}
                                </div>
                            ))
                        ) : (
